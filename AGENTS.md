...
The Android client lives under `android/app/`. Keep backend protocol and network code under `data/`, immutable client models under `model/`, deterministic tutorial walking and safety rules under `execution/`, and Compose screens plus ViewModels under feature packages in `ui/`. UI depends on repository interfaces rather than HTTP implementations and must not advance graph state independently of the execution engine. Keep Gradle Wrapper files and `gradle/libs.versions.toml` tracked; never commit `local.properties`, `.gradle/`, build outputs, APKs, emulator state, or screenshots.
