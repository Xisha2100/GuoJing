...
本模块完成时 Android 尚未接入新协议；后续模块 60 已完成客户端接入，见 [Android 智能体客户端](60-android-agent-client.md)。
...
主智能体、界面分析和隔离审校均接收由用户目标计算的语言规则。默认使用简体中文；只有用户目标含拉丁字母（含全角形式）时允许保留相关英文。截图、包名和历史步骤中的英文不构成例外，协议字段及枚举不翻译。

领域层在返回前检查所有状态的说明。中文目标下若模型仍输出英文字母，结果降级为 `cannot_determine`，清除目标框并使用固定中文重试说明，不进行可能改变操作含义的字符串删改。此检查不负责繁简转换；简体中文及英文例外的相关性由提示词与审校约束。离线测试覆盖中文、夹杂英文、全角英文、英文目标和三个结果状态。
