...
模型异常、网络超时、JSON 解析失败或领域规则拒绝，都会变成统一的 `needs_human_review` 结果。错误细节不直接暴露给老年用户，也不会因为重试而自动重复第三方 APP 操作。教程路由也被适配器拒绝，必须先由模块 23 完成证据和版本匹配。
...
模块 21–25 至此形成一条离线可测试链路：持久化结果 → 受控证据 → 教程匹配 → 编排状态 → 模型候选校验。真正接入模型时，仍必须沿用这五层边界。
