...
> MVP 基线已完成。仓库具备后端求助与教程状态图、版本发布、模板导入、管理端认证审计、React 管理网页，以及能够读取公开教程、观察目标页面、跨 APP 框选操作目标并验证低风险结果的 Android 客户端。Android 支持用户主动导入截图、在本机生成隐私遮挡副本，并在显式确认后发送受限求助请求；基础指引使用确定性安全目录，Deep Agent 适配端口已预留但默认不连接外部模型。
...
- 管理员只能发布经过领域规则校验的人工指引；复核接口仅展示有长度上限的问题正文，不展示截图或 OCR。
...
当前已具备“Android 提交 → 后端受控处理 → 管理网页查看/处理 → Android 轮询展示”的基础链路，并提供首批 13 个场景模板；发布接口会阻断未验证、非本地或非低风险修订。录制教程求助的候选发现、数据库级分布式 claim/lease/retry、真实 Deep Agent 和第三方 APP 持续适配仍未形成生产闭环，不应把当前版本用于真实用户或多进程部署。
