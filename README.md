...
> 当前处于早期 MVP 开发阶段。仓库已经具备后端基础骨架、教程状态图、版本发布、教程编辑工作区、管理端认证审计、React 管理网页，以及能够读取公开教程、观察目标页面、跨 APP 框选操作目标并验证低风险结果的 Android 客户端。Android 现已支持用户主动导入截图、在本机生成隐私遮挡副本，并在显式确认后发送受限求助请求；本地 ML Kit OCR 已接入 provider，但尚未自动替用户遮挡或发送，模型回答与 Agent 编排尚未接入。
...
下一模块将在截图求助页面中展示本地 OCR 识别出的隐私区域建议；每个建议都必须由用户确认，不能自动遮挡后直接发送。解析结果仍只作为建议和候选教程匹配证据，不能绕过“用户亲自操作、页面证据验证、金融/不可逆动作拦截”的确定性安全策略。
