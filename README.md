...
> 当前处于早期 MVP 开发阶段。仓库已经具备后端基础骨架、教程状态图、版本发布、教程编辑工作区、管理端认证审计、React 管理网页，以及能够读取公开教程、观察目标页面、跨 APP 框选操作目标并验证低风险结果的 Android 客户端。Android 现已支持用户主动导入截图、在本机生成隐私遮挡副本，并在显式确认后发送受限求助请求；OCR 目前只完成可替换证据合同，真实 OCR、模型回答与 Agent 编排尚未接入。
...
下一模块将在说明新增依赖和环境影响后，使用受控样本比较并选择第一个真实 OCR provider。解析结果仍只作为建议和候选教程匹配证据，不能绕过“用户亲自操作、页面证据验证、金融/不可逆动作拦截”的确定性安全策略。
