...
MVP 已具备“Android 提交 → 后端批处理 → 管理网页查看/处理 → Android 展示安全结果”的最小闭环，并提供首批 13 个场景模板。模板必须在真实设备上录制和验证后才能发布；默认基础指引不调用外部模型。真实 Deep Agent、第三方 APP 的持续版本维护、家属多账号和生产级分布式队列属于后续迭代。
