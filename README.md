...
> 当前处于早期 MVP 开发阶段。仓库已经具备后端基础骨架、教程状态图、版本发布、教程编辑工作区、管理端认证审计、React 管理网页，以及能够读取公开教程、观察目标页面、跨 APP 框选操作目标并验证低风险结果的 Android 客户端。Android 现已支持用户主动导入截图并在本机生成隐私遮挡副本；OCR、真实文件存储、联网求助与 Agent 编排尚未接入。
...
下一模块将建立截图求助请求契约与显式发送确认：后端只接收经过本地脱敏的副本，区分“匹配已录制教程”和“没有教程时生成基础解释”两条路径，并为临时对象、保留时间和审计信息划定边界。在选择 OCR/视觉服务和引入新的 Gradle/Python 依赖前，会先说明方案与环境影响；Agent 编排不会绕过现有执行安全策略。
